...
function ENT:OnThink()
	...
	if(self:slvIsOnFire()) then self:SetIdleActivity(ACT_IDLE_ON_FIRE)
...
function ENT:EventHandle(...)
	...
	if(event == "jump") then
		...
		else self:SLVPlayActivity(ACT_GLIDE) end
		return true
	elseif(event == "rattack") then
		...
		local vTarget = (self:SLV_IsPossesed() && self:GetPossessor():GetPossessionEyeTrace().HitPos) || self:GetPredictedEnemyPosition() || (att.Pos +self:GetForward() *500)
		...
		if(self:CanSee(enemy)) then
			...
			if(dist >= 160 && math.abs(self:GetForward():DotProduct((enemy:GetPos() -self:GetPos()):GetNormal())) > 0.91) then
				if(dist >= 360 && CurTime() >= self.m_nextJump && dist <= self.fRangeDistanceSpit) then
					...
					if(math.random(1,5) <= 3) then self:SLVPlayActivity(ACT_JUMP,true) end
					return
				end
