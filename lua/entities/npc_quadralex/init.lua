...
function ENT:SetupSLVFactions()
	self:SetNPCFaction(NPC_FACTION_MUTANT,CLASS_MUTANT)
end
...
function ENT:Charge()
	if self.m_bCharge then
		self:SLVPlayActivity(ACT_RUN_AIM_RELAXED)
		return
	end
	--if self.m_bCharging then return end
	self:SLVPlayActivity(ACT_RUN_AIM)
...
function ENT:OnThink()
	...
	if self.m_actCharge then self:SLVPlayActivity(self.m_actCharge,self.m_bChargeFaceEnemy) end
			...
			if IsValid(self.entEnemy) then
				...
				if self.m_tHitTarget && CurTime() -self.m_tHitTarget >= 1 then
					...
					if bCrash || tr.Hit then
						if IsValid(tr.Entity) && (tr.Entity:IsNPC() || tr.Entity:IsPlayer()) && tr.Entity:Health() > 0 && (self:slvDisposition(tr.Entity) == D_HT || self:slvDisposition(tr.Entity) == D_FR) then
							if !self.m_tHitTarget then
								self:slvPlaySound("ChargeHit")
					...
					elseif !self.m_tHitTarget then
						...
						if dist >= 200 && (yawDiff <= -50 || yawDiff >= 50) then
							...
							self:SLVPlayActivity(ACT_DOD_RUN_AIM)
						end
					end
				end
			end
...
function ENT:Crash()
	...
	self:EndCharge()
	self.m_tHitTarget = nil
	self:SLVPlayActivity(ACT_RUN_AIM_AGITATED)
	self:SetChargeActivity()
end
...
function ENT:EventHandle(...)
	...
	if(event == "rattack") then
		...
		if(IsValid(self.entEnemy) || self:SLV_IsPossesed()) then
			local posSelf = self:GetPos()
			if(!self:SLV_IsPossesed()) then posTgt = self.entEnemy:GetCenter() +self.entEnemy:GetVelocity() *0.5
			else
